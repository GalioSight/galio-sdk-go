...
func BenchmarkInject(b *testing.B) {
	var t internal.TraceContext

	injectSubBenchmarks(
		b, func(ctx context.Context, b *testing.B) {
			h := http.Header{}
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
...
func injectSubBenchmarks(b *testing.B, fn func(context.Context, *testing.B)) {
	b.Run(
		"SampledSpanContext", func(b *testing.B) {
			b.ReportAllocs()
	...
	b.Run(
		"WithoutSpanContext", func(b *testing.B) {
			b.ReportAllocs()
...
func BenchmarkExtract(b *testing.B) {
	extractSubBenchmarks(
		b, func(b *testing.B, req *http.Request) {
			var propagator internal.TraceContext
			ctx := context.Background()
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
...
func extractSubBenchmarks(b *testing.B, fn func(*testing.B, *http.Request)) {
	...
	b.Run(
		"FutureAdditionalData", func(b *testing.B) {
			req, _ := http.NewRequest("GET", "http://example.com", nil)
