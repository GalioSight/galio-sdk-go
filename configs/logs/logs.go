...
type option func(l *configs.Logs)

// WithExporter 设置 MetricsExporter.
// Deprecated: 直接覆盖整个对象有可能会导致配置错误，使用 ocp.WithLocalDecoder 代替。
func WithExporter(exporter *model.LogsExporter) option {
	return func(l *configs.Logs) {
		l.Exporter = *exporter
	}
}

// WithProcessor 设置 LogsProcessor.
// Deprecated: 直接覆盖整个对象有可能会导致配置错误，使用 ocp.WithLocalDecoder 代替。
func WithProcessor(processor *model.LogsProcessor) option {
	return func(l *configs.Logs) {
...
func NewConfig(
	...
	m := &configs.Logs{
		...
		Exporter:    config.LogsConfig.Exporter,
		Processor:   config.LogsConfig.Processor,
