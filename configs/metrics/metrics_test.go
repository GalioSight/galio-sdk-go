...
func TestNewMetricsConfig(t *testing.T) {
	...
	metrics := NewConfig(
		resource, WithConvertName(false),
		WithExporter(&model.MetricsExporter{}),
		WithProcessor(&model.MetricsProcessor{}),
		WithSchemaURL(semconv.SchemaURL),
	)
	assert.False(t, metrics.ConvertName)
	assert.Equal(t, "", metrics.Exporter.Protocol)
	assert.Equal(t, "", metrics.Processor.Protocol)
	assert.Equal(t, semconv.SchemaURL, metrics.SchemaURL)
}
